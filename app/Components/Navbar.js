    ...
    <div className={`ml-${isOpen ? '10' : '2'} transition-all duration-300`}>{/* Increase ml value when isOpen */}
      ...
      >
        ...
        <div className={`w-6 h-6 relative ${isOpen ? 'rotate-180' : ''} transition-transform duration-500`}>
          <span className={`absolute w-full h-0.5 bg-gray-800 transform transition-all duration-300 ease-in-out ${isOpen ? 'rotate-45 translate-y-2.5' : ''}`}></span>
          <span className={`absolute w-full h-0.5 bg-gray-800 transform transition-all duration-300 ease-in-out ${isOpen ? 'opacity-0' : 'translate-y-2'}`}></span>
          <span className={`absolute w-full h-0.5 bg-gray-800 transform transition-all duration-300 ease-in-out ${isOpen ? '-rotate-45 translate-y-2.5' : 'translate-y-4'}`}></span>
        </div>
      </button>
    </div>
  );
...
function MobileNavLink({ href, children, delay }) {
  return (
    <Link href={href}>
      <div className={`block text-gray-600 hover:text-blue-600 hover:bg-gray-100 py-2 px-4 cursor-pointer font-outfit transition-all duration-300 ease-in-out rounded-md animate-slideIn ${delay}`}>
        {children}
      </div>
    </Link>
  );
